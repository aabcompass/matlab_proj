...
for j=1:100000
    ...
    for i = 1:n_ecasic % 6
         pdm_2d_pc((i-1)*8+1:i*8,:) = pdm_2d_remap((i-1)*10+1:i*10-2,:); %показания с прибора
         pdm_2d_ki(i,:) = pdm_2d_remap(i*10-1,:);
    end;
    
    if fix_color_map==1
        %imagesc(a1,double(pdm_2d_remap/50000), clims);
        imagesc(a1,double(pdm_2d_pc)/integration, clims);colorbar(a1);
        imagesc(a2,double(pdm_2d_ki)/integration, clims);colorbar;
    else
        imagesc(a1,double(pdm_2d_pc)/integration);colorbar(a1);
